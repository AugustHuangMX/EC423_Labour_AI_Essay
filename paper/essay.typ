...
where $0 < sigma < 1$ is the within-occupation task substitution elasticity and $beta_L, beta_H, beta_G > 0$ are task share parameters summing to unity. For normalization, we let $beta_L + beta_H + beta_G = 1$. The parameter $sigma$ governs how readily the occupation substitutes between task types; it is conceptually distinct from the between-occupation elasticity $lambda$ in #cite(<NBERw33941>, form: "prose").
...
To measure occupation-level exposure to AI, I use the LLM exposure scores constructed by #cite(<GPTs_are_GPTs>, form: "prose"), who employ a combination of human annotators and GPT-4 to assess, for each O\*NET task, the share of task time that could be reduced by at least 50 per cent with access to a large language model. The resulting score enters all specifications as a continuous variable. I merge these scores to the Revelio positions data via O\*NET--SOC codes, achieving an 89.5 per cent match rate; unmatched positions are excluded from the analysis. The mobility regressions use the raw score rather than a z-score. In the extensive-margin estimation sample, this raw score ranges from 0 to 1, with mean 0.494, standard deviation 0.177, and interquartile range 0.147. I therefore interpret the mobility coefficients using one-standard-deviation and interquartile-range changes, rather than a full one-unit change. For robustness checks, I replicate the main specifications using two alternative measures: the AI Occupational Exposure index of #cite(<felten2021occupational>, form: "prose") and the AI Applicability Score of #cite(<tomlinson2025working>, form: "prose"), with results reported in @B.
...
Varying the seniority threshold reveals that the effect is concentrated around the junior--senior boundary rather than at the very lowest entry level. The coefficient is statistically insignificant when junior status is restricted to seniority level 1 alone, strongest at the baseline cutoff of $lt.eq 2$, and attenuated when expanded to $lt.eq 3$. This pattern should be interpreted cautiously, since the theoretical model does not distinguish between seniority levels 1 and 2. One possibility is that level-1 positions are too rare or too noisy to detect precise effects. Another is that level-1 roles, such as internships or formal trainee positions, are less exposed to the substantive routine cognitive tasks captured by the model than level-2 junior roles. The baseline result should therefore be read as evidence for displacement among workers below the expertise threshold, not a specific type alone.
...
@tab:transition reports results from the extensive-margin analysis. Using the full sample of 27.5 million positions (including those without a subsequent transition), I estimate the triple-difference specification with an indicator for whether a transition is observed as the dependent variable. The estimate of $beta_4$ is $0.548$. Because the mobility regressions use the raw LLM exposure score, this coefficient represents a full-scale increase from zero to one. A more meaningful comparison is a one-standard-deviation increase in exposure, which implies a 9.7 percentage-point increase in senior transition probability ($0.548 times 0.177$); an interquartile-range increase implies an 8.1 percentage-point increase. This pattern is compatible with an informal extension of the model in which higher marginal products raise outside options and poaching, but the formal model itself does not contain this mobility channel. It could also reflect senior workers leaving deteriorating matches in exposed occupations. I therefore interpret this coefficient as evidence of increased gross senior reallocation, not as a direct measure of net senior employment growth.

The triple-interaction term $beta_5 = -0.660$ indicates that the AI-exposure gradient in post-period mobility is substantially weaker for juniors than for seniors. Combining $beta_4$ and $beta_5$ gives $beta_4 + beta_5 approx -0.11$ per full-scale unit, or about -2.0 percentage points for a one-standard-deviation increase in exposure. This is a relative AI-exposure effect for juniors, not an absolute decline in junior mobility. The table also shows a large positive coefficient on $"Junior" times "Post"$ ($0.377$), implying a positive junior-specific post differential relative to seniors. Within the reported interaction terms, the post-period marginal effect for a junior in a fully exposed occupation is $0.377 + 0.548 - 0.660 approx 0.265$, and at the 75th percentile of AI exposure it is approximately $0.377 + (0.548 - 0.660) times 0.576 approx 0.312$. Thus the reported interaction components imply a positive junior post differential, while the AI-specific differential is negative. The absolute pre-post change also includes the common month fixed effects and is therefore not given by these coefficients alone. This seniority asymmetry is consistent with the model's formal junior-displacement implication, although the senior-side mechanism remains ambiguous without net flow evidence. The result is robust to excluding positions that began after July 2024 ($beta_5 = -0.662$), suggesting that the censoring of recently started positions does not drive the estimate. This test does not eliminate all right-censoring concerns, since older positions that remain ongoing at the data endpoint are still coded as non-transitions. The extensive-margin estimates should therefore be interpreted as transition probabilities observed within the current panel window rather than completed career-spell outcomes.
