...
#let essay-title     = "When Nothing Changes, Who Changes? AI Exposure and Early-Career Employment in UK Occupations"  // Your essay title
...
The remainder of this essay proceeds as follows. Section 2 develops the theoretical framework. Section 3 describes the data and identification strategy. Section 4 presents the empirical results. Section 5 discusses limitations, and Section 6 concludes.
...
/// 
= Discussion
A natural question is why I do not detect the wage movements predicted by the model. In the theory, junior employment falls because $w_J$ falls, while senior employment rises because $w_S$ rises. A high labour supply elasticity $epsilon$ can make wage changes small relative to quantity changes, but it does not remove the price response altogether. The wage null should therefore be read as a limitation of the empirical test rather than as confirmation of the wage predictions. One important caveat is that Revelio salaries are predicted from job titles, locations, and company characteristics, rather than directly observed compensation. This means the wage evidence is available, but less reliable than the employment and mobility evidence. Better administrative wage data would help distinguish true wage nulls from delayed effects or measurement smoothing.

The Heckman correction gives a related lesson. The inverse Mills ratio is negative ($hat(lambda) = -0.129$), suggesting negative selection on the outcome dimension: workers more likely to transition tend to have smaller seniority gains. The key point, however, is that the triple-interaction estimate is almost unchanged after correction ($0.125$ versus $0.122$). Since the corrected standard errors do not include Murphy--Topel or bootstrap adjustment, I place more weight on the stability of the point estimate than on its exact precision. The preferred interpretation is genuine skill-upgrading among transitioning juniors, although this remains a career-progression finding beyond the static model. The occupation-switching null supports this reading: juniors in high-exposure occupations are not mainly fleeing into unrelated occupations, but moving upward within related occupational space.

The extensive-margin result should also be interpreted cautiously. Senior workers in high-AI-exposure occupations become more mobile after ChatGPT, while the AI-specific mobility gradient is weaker for juniors. This does not mean that junior transition probabilities fall in absolute terms: the coefficient $beta_4 + beta_5$ only captures how the post-period mobility effect changes with AI exposure. More importantly, the senior result is a gross transition measure, not a net stock measure. A senior transition may reflect poaching into a better match, which is consistent with augmentation, or exit from a worse match. Distinguishing these channels would require occupation-level inflows, outflows, and net senior headcount changes. Right-censoring also remains a limitation: excluding very recent position starts is useful, but a stronger test would impose a common follow-up window or estimate a duration model.

With richer linked employer--employee data, a further extension would be to use the theoretical model more quantitatively. Direct measures of task allocation, compensation, and occupation-level flows could help discipline the task substitution elasticity, labour supply elasticity, and task weights. This would make it possible to quantify the displacement and augmentation channels, rather than only signing them.

A broader concern is the lack of consensus on how to measure occupational exposure to AI. #cite(<gimbel2026labor>, form: "prose") show that common indices disagree on which occupations are most exposed, partly because they define exposure through different objects: task descriptions, technical capabilities, patents, or observed use. This motivates my use of three measures: the primary LLM exposure score, the AI Occupational Exposure index, and the AI Applicability Score. The last should be interpreted cautiously because it is based on observed Copilot use during the post-treatment period. It is useful as a revealed-use comparison, but weaker as an exogenous exposure measure. A separate data limitation is that Revelio likely over-represents white-collar professional workers, where LLM exposure is especially concentrated. This may amplify the estimated effects relative to a nationally representative labour-force dataset.

Finally, the seniority-cutoff robustness checks suggest that the effect is strongest around the junior--senior boundary, not at the very lowest entry level. One possible interpretation is that AI exposure bites most sharply after entry-level training but before workers cross the expertise threshold into more autonomous tasks. Level-2 workers may perform the drafting, coordination, coding, summarisation, and analytical support tasks that current AI systems can substitute for, while level-1 roles may be partly protected by training pipelines, supervision requirements, or noisier job-title classification. This distinction between entry-level access and early-career progression is important for future work. More broadly, these estimates capture the early stages of generative AI diffusion. As AI capabilities move from text-based assistance toward more autonomous task execution, current occupational task bundles may not remain stable. Longer post-treatment panels will be needed to show whether these compositional shifts persist, intensify, or reverse.

These limitations do not overturn the main result, but they define its scope. The paper is best read as evidence on early, within-occupation adjustment to generative AI, rather than as a complete account of long-run labour-market effects.

In this sense, the results are most useful as a warning about where adjustment may first appear. Aggregate occupation totals may remain stable even while entry-level pathways inside those occupations narrow.
...
These findings carry a subtler implication. Much of the public debate frames AI as a threat to entire occupations or skill groups. The evidence here suggests a different pattern: AI reshapes _who does the work within an occupation_ before it reshapes _whether the occupation exists_. If this pattern generalises, the workers most immediately affected are those at the bottom of the ladder within occupations that may still be thriving overall. This is a form of displacement that aggregate statistics are poorly equipped to detect, and that conventional retraining policies may be poorly designed to address. The relevant policy question may therefore be how firms and institutions restructure entry-level pathways when the tasks that once served as training grounds are increasingly performed by machines.
