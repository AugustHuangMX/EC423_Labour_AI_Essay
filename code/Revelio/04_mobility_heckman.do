/*
============================================================
04_mobility_heckman.do
============================================================
目的：Career Mobility 分析，构建论文 Empirical Results 中
      Mobility subsection 所需的全部回归结果和表格。

逻辑：
  Part 1 — Intensive Margin（Heckman Two-Step）
    1a. 构建公司级 pre-period separation rate 作为 exclusion restriction
    1b. 在 10% sample 上跑 probit（全量太慢），手动计算 IMR
    1c. Outcome regressions: delta_seniority, delta_log_salary, occ_switch
        分别跑 baseline（无 IMR）和 Heckman-corrected（有 IMR）版本
    → 导出 Table 1: 主回归结果（6 列）

  Part 2 — Extensive Margin
    2a. 描述性：transition rate by junior × ai_group × post
    2b. Triple-diff regression: P(transition) as outcome
    2c. Robustness: 排除 2024 年 7 月后开始的 position
    → 导出 Table 2: extensive margin 回归结果（2 列）

  Part 3 — 描述性统计
    3a. Transition sample 的关键变量 summary statistics
    → 导出 Table 3: 描述性统计

导出表格清单（论文中需要体现的）：
  - Table 1 (正文): 主回归结果，6 列
      Col 1-2: delta_seniority (baseline / Heckman)
      Col 3-4: delta_log_salary (baseline / Heckman)
      Col 5-6: occ_switch (baseline / Heckman)
  - Table 2 (正文): extensive margin，2 列
      Col 1: full sample, Col 2: robustness (exclude recent)
  - Table 3 (正文或 appendix): 描述性统计
  - Table 4 (appendix): transition rate by junior × ai_group × post
============================================================
*/
...
global output "/Users/huangminxing/Documents/EC423_Essay/output/tables"

* 确保 output 文件夹存在
capture mkdir "$output"

* --- 辅助 program：根据 t 值返回星号 ---
...
* 导出 Table 3
putexcel set "$output/table3_summary_stats.xlsx", replace
putexcel A1 = "Variable" B1 = "N" C1 = "Mean" D1 = "SD" E1 = "p25" F1 = "p50" G1 = "p75"
putexcel A2 = "Panel A: Full Transition Sample"
putexcel A3 = matrix(A'), names
putexcel A10 = "Panel B: Junior (seniority <= 2)"
putexcel A11 = matrix(B'), names
putexcel A18 = "Panel C: Senior (seniority > 2)"
putexcel A19 = matrix(C'), names
...
replace post = (startdate >= td(01nov2022)) if missing(post) & !missing(startdate)
...
file open fh using "$output/table1_mobility_regressions.csv", write replace

file write fh "Variable,Sen_Base,Sen_Base_SE,Sen_Heck,Sen_Heck_SE,"
...
foreach v of local vars {
    ...
    foreach m in base_sen heck_sen base_sal heck_sal base_occ heck_occ {
        ...
        get_stars `b' `s'
        local st = r(stars)
        file write fh "," %9.4f (`b') "`st'" "," %9.4f (`s')
    }
    file write fh _n
}

* delta_seniority（只在 salary 模型里）
file write fh "delta_seniority,,,,"
estimates restore base_sal
local b = _b[delta_seniority]
local s = _se[delta_seniority]
get_stars `b' `s'
file write fh "," %9.4f (`b') "`r(stars)'" "," %9.4f (`s')
estimates restore heck_sal
local b = _b[delta_seniority]
local s = _se[delta_seniority]
get_stars `b' `s'
file write fh "," %9.4f (`b') "`r(stars)'" "," %9.4f (`s')
